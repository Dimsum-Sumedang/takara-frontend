...
- Mengunci input mikrofon selama Takara memproses giliran aktif untuk mencegah request paralel.
...
- Takara Backend yang sudah berjalan.
...
4. Isi `.env`:
   ...
   ```env
   TAKARA_API_BASE_URL=http://localhost:8000/api/v1
...
|---|---:|---|---|
| `TAKARA_API_BASE_URL` | Ya | Server-only | Base URL FastAPI beserta prefix `/api/v1`. |
...
JWT_AUDIENCE=takara-web
JWT_ISSUER=takara-api
```
