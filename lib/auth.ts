...
export async function getCurrentUser(): Promise<AuthUser | null> {
  ...
  const userStr = localStorage.getItem("takara_user")
  if (userStr) {
    try {
...
export async function getAccessToken(): Promise<string | null> {
  if (typeof window === "undefined") return null
  return localStorage.getItem("takara_token")
}
...
export async function signIn(
  ...
  try {
    ...
    localStorage.setItem("takara_token", data.access_token)
    localStorage.setItem("takara_user", JSON.stringify(data.user))
    notifyAuthChange()
...
export async function signOut() {
  if (typeof window !== "undefined") {
      localStorage.removeItem("takara_token")
      localStorage.removeItem("takara_user")
      notifyAuthChange()
  }
}
