...
function SessionInner() {
  ...
  const handlePressStart = useCallback(() => {
    ...
    // Allow interrupting while Takara is thinking or speaking — hard-stop first.
  ...
  const handleDesktopToggle = useCallback(() => {
    if (phase === "thinking" || phase === "speaking") {
      // Interrupt Takara and start a new recording immediately.
      hardStopPlayback()
      setAiText("")
      setPhase("idle")
      pressedRef.current = true
  ...
  // While recording, the orb mirrors mic RMS in real time. While Takara is
      ...
      <main className="hidden lg:grid grid-cols-[1.05fr_1fr] gap-12 xl:gap-16 items-center px-8 pb-12 min-h-[calc(100dvh-110px)]">
        ...
        <div className="space-y-6 max-w-xl min-w-0">
          ...
          <div className="space-y-2 min-w-0">
            <p className="text-[12px] uppercase tracking-[0.18em] text-muted-foreground">
              {userText ? "Takara" : "Live transcript"}
            </p>
            <div
